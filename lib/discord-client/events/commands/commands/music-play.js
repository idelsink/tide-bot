...
const play = async ({message}) => {
  ...
  if (!song) {
    ...
    embed.setAuthor(song.videoDetails.author.name,
        _.last(song.videoDetails.author.thumbnails).url,
        song.videoDetails.author.channel_url ||
        song.videoDetails.author.user_url);
    embed.setTitle(`:musical_note: Now playing :musical_note:`);
    embed.setDescription(song.videoDetails.title);
    embed.setURL(song.videoDetails.video_url);
    embed.setThumbnail(_.last(song.videoDetails.thumbnail.thumbnails).url);
    // Show queue
  ...
  const dispatcher = serverQueue.connection
      .play(await ytdl(song.videoDetails.video_url), {type: 'opus'});
  log.debug(`Playing song ${song.videoDetails.title}`);
  ...
  execute: async ({message, args}) => {
    ...
    if (!serverQueue) {
      const queueContruct = {
        ...
        connection: null,
        songs: [],
        volume: 0.4, // 40% volume seems the best default in the channel
        playing: true,
      };
      ...
      if (_.size(songInfos) > 1) {
        ...
        return await message.reply(
            `${_.first(songInfos).videoDetails.title}`+
